...
class StudentProfileCustomFieldsTest extends TestCase
{
    /** @var list<int> */
    ...
    private function actingAsSuperAdmin(): void
    {
        ...
        $token = uniqid('spcf', true);
        $staffId = DB::table('staff')->insertGetId([
            'employee_id' => 'SPCF-'.$token,
            'lang_id' => 1,
            'currency_id' => 0,
            'qualification' => '',
            'work_exp' => '',
    ...
    public function test_student_profile_shows_visible_on_table_custom_fields(): void
    {
        $this->actingAsSuperAdmin();
        $suffix = uniqid();

        $session = AcademicSession::query()->first()
            ?: AcademicSession::query()->create(['session' => '2095-spcf']);
        DB::table('sch_settings')->limit(1)->update(['session_id' => $session->id]);
        app(SchoolContext::class)->clearCache();

        $section = Section::query()->create(['section' => 'SP-'.$suffix, 'is_active' => 'yes']);
        $class = SchoolClass::query()->create(['class' => 'SPC-'.$suffix, 'is_active' => 'yes']);
        ...
        $visibleName = 'Profile CF '.$suffix;
        $hiddenName = 'Profile Hidden '.$suffix;
        ...
        $this->post('/student/create', [
            ...
            'custom_fields' => [
                'students' => [
                    $visible->id => 'ProfileValue-'.$suffix,
                    $hidden->id => 'HiddenProfile-'.$suffix,
                ],
            ],
        ])->assertRedirect();
        ...
        $response = $this->post('/report/student_profile', [
            'class_id' => $class->id,
            'section_id' => $section->id,
        ])->assertOk();

        $response->assertSee($visibleName, false);
        $response->assertSee('ProfileValue-'.$suffix, false);
        $response->assertDontSee($hiddenName, false);
        $response->assertDontSee('HiddenProfile-'.$suffix, false);
    }
}
