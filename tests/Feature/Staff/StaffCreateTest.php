<?php

namespace Tests\Feature\Staff;

use App\Modules\Staff\Models\Staff;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\File;
use Tests\TestCase;

class StaffCreateTest extends TestCase
{
    /** @var list<int> */
    private array $createdStaffIds = [];

    protected function tearDown(): void
    {
        foreach ($this->createdStaffIds as $staffId) {
            DB::table('custom_field_values')->where('belong_table_id', $staffId)->delete();
            DB::table('staff_leave_details')->where('staff_id', $staffId)->delete();
            DB::table('staff_roles')->where('staff_id', $staffId)->delete();
            DB::table('staff')->where('id', $staffId)->delete();

            foreach (['barcodes', 'qrcode'] as $folder) {
                $path = public_path('uploads/staff_id_card/'.$folder.'/'.$staffId.'.png');
                if (File::exists($path)) {
                    File::delete($path);
                }
            }
        }
        $this->createdStaffIds = [];

        parent::tearDown();
    }

    private function actingAsSuperAdmin(): int
    {
        $roleId = (int) (DB::table('roles')->where('is_superadmin', 1)->value('id')
            ?: DB::table('roles')->where('name', 'Super Admin')->value('id'));
        $this->assertGreaterThan(0, $roleId);

        $token = uniqid('stc', true);
        $staffId = DB::table('staff')->insertGetId([
            'employee_id' => 'STC-'.$token,
            'lang_id' => 1,
            'currency_id' => 0,
            'qualification' => '',
            'work_exp' => '',
            'name' => 'Super',
            'surname' => 'Admin',
            'father_name' => '',
            'mother_name' => '',
            'contact_no' => '',
            'emergency_contact_no' => '',
            'email' => $token.'@example.test',
            'dob' => '1990-01-01',
            'marital_status' => '',
            'local_address' => '',
            'permanent_address' => '',
            'note' => '',
            'image' => '',
            'password' => bcrypt('secret'),
            'gender' => 'Male',
            'account_title' => '',
            'bank_account_no' => '',
            'bank_name' => '',
            'ifsc_code' => '',
            'bank_branch' => '',
            'payscale' => '',
            'epf_no' => '',
            'contract_type' => '',
            'shift' => '',
            'location' => '',
            'facebook' => '',
            'twitter' => '',
            'linkedin' => '',
            'instagram' => '',
            'resume' => '',
            'joining_letter' => '',
            'resignation_letter' => '',
            'other_document_name' => '',
            'other_document_file' => '',
            'user_id' => 0,
            'is_active' => 1,
            'verification_code' => '',
        ]);
        DB::table('staff_roles')->insert([
            'staff_id' => $staffId,
            'role_id' => $roleId,
            'is_active' => 1,
        ]);
        $this->createdStaffIds[] = $staffId;
        $this->actingAs(Staff::query()->findOrFail($staffId), 'staff');

        return $roleId;
    }

    public function test_staff_create_form_and_persist(): void
    {
        $teacherRoleId = (int) (DB::table('roles')->where('name', 'Teacher')->value('id')
            ?: DB::table('roles')->where('id', '!=', DB::table('roles')->where('is_superadmin', 1)->value('id'))->value('id'));
        $this->assertGreaterThan(0, $teacherRoleId);

        $this->actingAsSuperAdmin();
        $suffix = uniqid();

        $this->get('/admin/staff/create')
            ->assertOk()
            ->assertSee(__('system.add_staff'), false)
            ->assertSee('name="role"', false);

        $payload = [
            'employee_id' => 'EMP'.$suffix,
            'role' => $teacherRoleId,
            'name' => 'New',
            'surname' => 'Staff',
            'gender' => 'Male',
            'dob' => '1991-05-15',
            'email' => 'staff'.$suffix.'@example.test',
            'contactno' => '03001234567',
            'date_of_joining' => '2026-01-01',
            'contract_type' => 'permanent',
        ];

        DB::table('sch_settings')->limit(1)->update(['staffid_auto_insert' => 0]);

        $this->post('/admin/staff/create', $payload)
            ->assertRedirect(route('staff.index'))
            ->assertSessionHas('success');

        $staff = Staff::query()->where('email', $payload['email'])->first();
        $this->assertNotNull($staff);
        $this->createdStaffIds[] = (int) $staff->id;

        $this->assertSame($payload['employee_id'], $staff->employee_id);
        $this->assertSame('New', $staff->name);
        $this->assertSame(1, (int) $staff->is_active);

        $roleRow = DB::table('staff_roles')
            ->where('staff_id', $staff->id)
            ->where('role_id', $teacherRoleId)
            ->first();
        $this->assertNotNull($roleRow);

        $this->assertTrue(
            File::exists(public_path('uploads/staff_id_card/barcodes/'.$staff->id.'.png'))
            || File::exists(public_path('uploads/staff_id_card/qrcode/'.$staff->id.'.png'))
        );
    }

    public function test_staff_create_rejects_duplicate_email(): void
    {
        $roleId = $this->actingAsSuperAdmin();
        $suffix = uniqid();
        $email = 'dup'.$suffix.'@example.test';

        DB::table('sch_settings')->limit(1)->update(['staffid_auto_insert' => 0]);

        $this->post('/admin/staff/create', [
            'employee_id' => 'DUP1-'.$suffix,
            'role' => $roleId,
            'name' => 'Dup',
            'gender' => 'Male',
            'dob' => '1990-01-01',
            'email' => $email,
        ])->assertRedirect(route('staff.index'));

        $first = Staff::query()->where('email', $email)->firstOrFail();
        $this->createdStaffIds[] = (int) $first->id;

        $this->from('/admin/staff/create')
            ->post('/admin/staff/create', [
                'employee_id' => 'DUP2-'.$suffix,
                'role' => $roleId,
                'name' => 'Dup2',
                'gender' => 'Male',
                'dob' => '1990-01-01',
                'email' => $email,
            ])
            ->assertSessionHasErrors('email');
    }
}
