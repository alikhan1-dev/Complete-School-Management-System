...
class PortalProcessingFeesTest extends TestCase
{
    /** @var list<int> */
    ...
    public function test_portal_shows_processing_button_and_modal_json(): void
    {
        $this->assertTrue(Schema::hasTable('student_fees_processing'));
        $this->assertTrue(Schema::hasTable('gateway_ins'));
        ...
        $section = Section::query()->create(['section' => 'SP-'.$suffix, 'is_active' => 'yes']);
        $class = SchoolClass::query()->create(['class' => 'CP-'.$suffix, 'is_active' => 'yes']);
        ClassSection::query()->create(['class_id' => $class->id, 'section_id' => $section->id, 'is_active' => 'yes']);

        $type = FeeType::query()->create([
            'type' => 'TP-'.$suffix, 'code' => 'CP-'.$suffix, 'description' => '', 'is_system' => 0, 'nature' => '', 'is_active' => 'no',
        ]);
        $group = FeeGroup::query()->create([
            'name' => 'GP-'.$suffix, 'description' => '', 'is_system' => 0, 'nature' => '', 'is_active' => 'no',
        ]);
        ...
        $processingId = DB::table('student_fees_processing')->insertGetId([
            'gateway_ins_id' => $gatewayId,
            'fee_category' => 'fees',
            ...
            'amount_detail' => json_encode([
                ...
                'payment_mode' => 'Online',
                'date' => '2026-08-20',
                'description' => 'Pending gateway',
            ]),
            'is_active' => 'no',
        ]);
        $this->cleanupProcessingIds[] = $processingId;
        ...
        $this->get('/user/user/getfees')
            ...
            ->assertSee(__('system.processing_fees'), false);

        $json = $this->postJson('/user/user/getProcessingfees')
            ->assertOk()
            ->assertJsonStructure(['view']);

        $view = (string) $json->json('view');
        $this->assertStringContainsString($uniqueId, $view);
        $this->assertStringContainsString('Pending gateway', $view);
        $this->assertStringContainsString('800.00', $view);
        $this->assertStringContainsString('200.00', $view);
    ...
    public function test_getfees_hides_processing_button_without_rows(): void
    {
        $this->actingAsSuperAdmin();
        $suffix = uniqid();
        ...
        $section = Section::query()->create(['section' => 'SN-'.$suffix, 'is_active' => 'yes']);
        $class = SchoolClass::query()->create(['class' => 'CN-'.$suffix, 'is_active' => 'yes']);
        ClassSection::query()->create(['class_id' => $class->id, 'section_id' => $section->id, 'is_active' => 'yes']);

        $admissionNo = 'ADN'.$suffix;
        $this->post('/student/create', [
            'admission_no' => $admissionNo,
            'firstname' => 'None',
            'lastname' => 'Proc',
        ...
        $this->get('/user/user/getfees')
            ->assertOk()
            ->assertDontSee('id="btn_get_processing_fees"', false);
