...
class FeeReceiptPrintTest extends TestCase
{
    /** @var list<int> */
    ...
    private function actingAsSuperAdmin(): void
    {
        ...
        $token = uniqid('frp', true);
        $staffId = DB::table('staff')->insertGetId([
            'employee_id' => 'FRP-'.$token,
            'lang_id' => 1,
            'currency_id' => 0,
            'qualification' => '',
            'work_exp' => '',
    ...
    public function test_print_fees_by_name_json_and_page(): void
    {
        ...
        $this->assertStringContainsString('Receipt note', $page);
        $this->assertStringContainsString($admissionNo, $page);
        $this->assertStringContainsString($deposit->id.'/1', $page);
        $this->assertStringContainsString('500.00', $page);
        $this->assertStringContainsString('25.00', $page);
        ...
        $this->get('/studentfee/printFeesByName?'.http_build_query([
            ...
            ->assertSee('Receipt note', false)
            ->assertSee('T-'.$suffix, false);

        $this->get('/studentfee/addfee/'.$studentSession->id)
            ->assertOk()
            ->assertSee('printFeesByName', false)
            ->assertSee('main_invoice='.$deposit->id, false);
    ...
    public function test_print_fees_by_name_missing_invoice_returns_404(): void
    {
        $this->actingAsSuperAdmin();

        $this->postJson('/studentfee/printFeesByName', [
            'main_invoice' => 99999999,
            'sub_invoice' => 1,
            'fee_category' => 'fees',
        ])->assertNotFound();
    }
