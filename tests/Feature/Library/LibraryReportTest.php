...
class LibraryReportTest extends TestCase
{
    ...
    private function actingAsSuperAdmin(): Staff
    {
        ...
        $token = uniqid('librpt', true);
        $staffId = DB::table('staff')->insertGetId([
            'employee_id' => 'LR-'.$token,
            'lang_id' => 1,
            'currency_id' => 0,
            'qualification' => '',
            'work_exp' => '',
    ...
    public function test_library_reports_hub_and_searches(): void
    {
        ...
        $openBook = Book::query()->create([
            'book_title' => 'Open Report Book '.$suffix,
            'book_no' => 'ORB-'.$suffix,
        ...
        $returnedBook = Book::query()->create([
            'book_title' => 'Returned Report Book '.$suffix,
            'book_no' => 'RRB-'.$suffix,
        ...
        $this->get('/report/library')
            ->assertOk()
            ->assertSee('Library Report', false)
            ->assertSee('Book Issue Report', false)
            ->assertSee('Book Due Report', false)
            ->assertSee('Book Inventory Report', false)
            ->assertSee('Book Issue Return Report', false);

        $this->get('/report/studentbookissuereport?search=1&search_type=this_year&members_type=teacher')
            ->assertOk()
            ->assertSee('Open Report Book '.$suffix, false)
            ->assertDontSee('Returned Report Book '.$suffix, false);

        $this->get('/report/bookduereport?search=1&search_type=this_year&members_type=teacher')
            ->assertOk()
            ->assertSee('Open Report Book '.$suffix, false);

        $this->get('/report/bookinventory?search=1&search_type=this_year')
            ->assertOk()
            ->assertSee('Open Report Book '.$suffix, false)
            ->assertSee('Returned Report Book '.$suffix, false);

        $this->get('/admin/book/issue_returnreport?search=1&search_type=this_year')
            ->assertOk()
            ->assertSee('Returned Report Book '.$suffix, false)
            ->assertDontSee('Open Report Book '.$suffix, false);
    }
}
