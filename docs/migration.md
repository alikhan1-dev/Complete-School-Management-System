...
| **7** | Communication, Chat, FrontCms, FrontOffice, OnlineAdmission | In progress (Communication persist + staff/user Chat persist/polling + FrontOffice persist slices done + FrontCms admin persist + public site done + OnlineAdmission admin persist + enroll + public form/review/submit/edit + checkout + applicant files + custom fields + enroll copy to student + enroll document/photo copy + enroll barcode/qrcode + admission captcha persist done; live send + live gateways + SaaS quota deferred) |
