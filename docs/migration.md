...
| **2** | Academics, Students (Parents/Timetable partial deferred) | Done (core Academics + Students; fees-on-admit + multi-class + class-teacher scopes + disabled-list extras + alumni list/details/events/custom-fields/calendar feed done; Parents credentials endpoints done — live gateway deferred; Timetable done; batch subjects skipped; deferred: alumni mail/SMS → Communication) |
...
| **3b** | Attendance (day / subject / staff) + remaining student history | Operational core done (day + by-date + subject/period + staff mark-save + period reportbydate + day/by-date + subject/period class-teacher scope + staff attendance superadmin_visible); deferred: SMS, biometric |
...
| **5** | Exams, OnlineExam, Certificates | In progress (Exams + Certificates done; OnlineExam admin + student take-exam + reports + ranking generation + question bank superadmin_visible creator masking done; mail deferred) |
| **6** | Library, Transport, Hostel, Inventory, Payroll, Leave, Homework, LessonPlan | In progress (Homework incl. homework report class-teacher scope + Library incl. book issue empty-matrix deny + library reports/member list superadmin_visible + admin issue report list + Payroll incl. superadmin_visible staff filtering + Leave incl. superadmin_visible filtering + Transport incl. student transport report class-teacher scope + Hostel incl. student hostel report class-teacher scope + Transport operational core + Hostel + Inventory + Leave incl. student approve_leave class-teacher scope done; LessonPlan core + full admin class-teacher scope incl. weekly matrix done; deferred: student portal comments) |
...
- [x] Staff: list + DataTables + superadmin_visible staff filtering + create/edit + profile (incl. payroll + photo + leave summary + rating) + attendance AJAX + documents (incl. create/edit upload) + timeline + disable/enable + delete + import + SaaS/credential hooks + teachers rating admin list + department/designation masters
