...
| **Attendance** | Staff attendance | `admin/Staffattendance.php` | `staff_attendance`, `staff_attendance_type`, `staff_attendence_schedules` | `app/Modules/Attendance/` | In progress (mark/save by role+date + profile month AJAX + superadmin_visible staff filtering done; deferred: SMS, biometric auto-mark) |
...
| **OnlineExam** | Online exams & question bank | `admin/Onlineexam.php`, `admin/Question.php` | `onlineexam`, `onlineexam_questions`, `onlineexam_students`, `questions` | `app/Modules/OnlineExam/` | In progress (admin core + student take-exam + reports incl. class-teacher scope + ranking generation + question bank superadmin_visible creator masking done; deferred: mail/SMS) |
...
| **Library** | Books & issues | `admin/Book.php`, `admin/Member.php` | `books`, `book_issues`, `libarary_members` | `app/Modules/Library/` | Done (admin + reports incl. book issue class-teacher empty-matrix deny + library reports/member list superadmin_visible + admin issue report list + portal) |
...
| **Payroll** | Payroll & payslips | `admin/Payroll.php` | `staff_payroll`, `staff_payslip`, `payslip_allowance` | `app/Modules/Payroll/` | Done (admin core + report incl. superadmin_visible staff filtering; deferred: currency helpers, print header image, SMS/mail) |
| **Leave** | Leave types & requests | `admin/Leavetypes.php`, `admin/Leaverequest.php`, `admin/Approve_leave.php` | `leave_types`, `staff_leave_request`, `staff_leave_details`, `student_applyleave` | `app/Modules/Leave/` | Done (types + staff approve/self-apply + student approve_leave incl. class-teacher scope + superadmin_visible filtering + reports; deferred: SaaS quota, mail/SMS) |
