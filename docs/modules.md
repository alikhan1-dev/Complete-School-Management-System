...
| **Students** | Disable / alumni | `admin/Disable_reason.php`, `Student.php` (`disablestudentslist`), `admin/Alumni.php` | `disable_reason`, `students`, `alumni_students`, `alumni_events` | `app/Modules/Students/` | In progress (disable reason + disabled students list + alumni list/details + events done; alumni mail/SMS + calendar deferred; disabled list class-teacher scope deferred) |
...
| **Fees** | Fee types, groups, masters | `Feetype.php`, `Feemaster.php`, `admin/Feegroup.php` | `feetype`, `feemasters`, `fee_groups`, `fee_groups_feetype`, `fee_session_groups`, `cumulative_fine` | `app/Modules/Fees/` | In progress (types/groups/master/assign + cumulative fine slabs + collect remaining-fine calc done) |
| **Fees** | Fee collection & discounts | `Studentfee.php`, `admin/Feediscount.php`, `admin/Feesforward.php` | `student_fees`, `student_fees_master`, `student_fees_deposite`, `student_fees_discounts`, `fees_discounts`, `student_applied_discounts` | `app/Modules/Fees/` | In progress (collect + multi + due-fees + carry-forward + transport single/multi collect + printFeesByName/ByGroup/ByGroupArray done; deferred: thermal print, SMS) |
| **Fees** | Transport fees | `Studentfee.php` | `student_transport_fees`, `transport_feemaster` | `app/Modules/Fees/` | In progress (ledger + single + multi deposit on addfee done; fees-master admin deferred to Transport) |
| **Fees** | Offline fee payments | `admin/Offlinepayment.php`, `user/Offlinepayment.php` | `offline_fees_payments` | `app/Modules/Fees/` | In progress (admin + portal submit + getfees offline entry done; deferred: online gateway pay modal, DataTables AJAX pixel-parity, SaaS quota) |
| **Fees** | Student portal fees | `user/User.php` (`getfees`) | (reads fee + transport ledgers; `student_fees_processing` + `gateway_ins`) | `app/Modules/Fees/` | In progress (getfees ledger + offline pay entry + previous-session flag + printFeesByName/ByGroupArray + processing-fee banner done; deferred: online payment modal) |
...
| **OnlineExam** | Online exams & question bank | `admin/Onlineexam.php`, `admin/Question.php` | `onlineexam`, `onlineexam_questions`, `onlineexam_students`, `questions` | `app/Modules/OnlineExam/` | In progress (admin core + student take-exam + reports + ranking generation done; deferred: mail/SMS) |
...
| **Homework** | Homework & daily assignments | `Homework.php`, `admin/` (homework views) | `homework`, `homework_evaluation`, `daily_assignment`, `submit_assignment` | `app/Modules/Homework/` | In progress (CRUD + evaluation + portal + daily + reports done; mail/SMS deferred to Communication) |
| **LessonPlan** | Lessons, topics, syllabus | `admin/Lessonplan.php`, `admin/Syllabus.php` | `lesson`, `topic`, `subject_syllabus`, `lesson_plan_forum` | `app/Modules/LessonPlan/` | In progress (lesson + topic + status + copy + weekly manage + admin forum done; deferred: student portal comments, class-teacher scope, DataTables AJAX, SaaS quota) |
| **Library** | Books & issues | `admin/Book.php`, `admin/Member.php` | `books`, `book_issues`, `libarary_members` | `app/Modules/Library/` | Done (admin + reports + portal; minor: admin issue_report list, superadmin_visible filter) |
| **Transport** | Vehicles, routes, pickup points | `admin/Vehicle.php`, `admin/Route.php`, `admin/Vehroute.php`, `admin/Pickuppoint.php` | `vehicles`, `transport_route`, `vehicle_routes`, `pickup_point`, `route_pickup_point` | `app/Modules/Transport/` | In progress (admin core + student transport report done; student fees/reorder/maps deferred) |
| **Hostel** | Hostels & rooms | `admin/Hostel.php`, `admin/Hostelroom.php`, `admin/Roomtype.php` | `hostel`, `hostel_rooms`, `room_types` | `app/Modules/Hostel/` | Done (admin core + student hostel report) |
...
| **Leave** | Leave types & requests | `admin/Leavetypes.php`, `admin/Leaverequest.php`, `admin/Approve_leave.php` | `leave_types`, `staff_leave_request`, `staff_leave_details`, `student_applyleave` | `app/Modules/Leave/` | Done (types + staff approve/self-apply + student approve_leave + reports; deferred: class-teacher scope, SaaS quota, mail/SMS) |
