...
| **Fees** | Student portal fees | `user/User.php` (`getfees`) | (reads fee + transport ledgers) | `app/Modules/Fees/` | In progress (getfees ledger + offline pay entry + previous-session flag + printFeesByName/ByGroupArray done; deferred: online payment modal, processing-fee banner) |
