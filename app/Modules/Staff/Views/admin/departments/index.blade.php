        ...
        <div class="col-md-4">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">
                        {{ $editing ? __('system.edit_department') : __('system.add_department') }}
                    </h3>
                </div>
                <form method="post" action="{{ route('staff.departments.index') }}">
                    @csrf
                    @if($editing)
                        <input type="hidden" name="departmenttypeid" value="{{ $editing->id }}">
                    @endif
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text"
                                   name="type"
                                   class="form-control"
                                   value="{{ old('type', $editing->department_name ?? '') }}">
                            @error('type')
    ...
    <div class="col-md-{{ ($showForm ?? false) ? '8' : '12' }}">
        <div class="box box-primary">
            <div class="box-header ptbnull">
                <h3 class="box-title">{{ __('system.department_list') }}</h3>
            </div>
            <div class="box-body">
                @if(session('success') && ! ($showForm ?? false))
                    <div class="alert alert-success">{{ session('success') }}</div>
                                ...
                                <td class="text-right">
                                    @if($canEdit ?? false)
                                        <a class="btn btn-primary btn-xs"
                                           href="{{ route('staff.departments.edit', $row->id) }}"
                                    ...
                                    @if($canDelete ?? false)
                                        <a class="btn btn-primary btn-xs"
                                           href="{{ route('staff.departments.destroy', $row->id) }}"
