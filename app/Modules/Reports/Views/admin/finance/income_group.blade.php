...
<div class="box box-primary">
    ...
    <form action="{{ url('financereports/incomegroup') }}" method="post">
        @csrf
        <div class="box-body">
            <div class="row">
                ...
                <div class="col-sm-6 col-md-3">
                    <div class="form-group">
                        <label>{{ __('system.income_head') }}</label>
                        <select class="form-control" name="head">
                            <option value="">{{ __('system.select') }}</option>
                            @foreach($headlist as $head)
                                <option value="{{ $head->id }}" @selected((string) $filters['head'] === (string) $head->id)>{{ $head->income_category }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
@if($searched)
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title"><i class="fa fa-money"></i> {{ __('system.income_group_report') }}</h3>
        </div>
