...
<div class="box box-primary">
    ...
    <form action="{{ url('report/parentlogindetailreport') }}" method="post">
        @csrf
        <div class="box-body">
            <div class="row">
...
<div class="box box-primary">
    <div class="box-header ptbnull">
        <h3 class="box-title titlefix"><i class="fa fa-users"></i> {{ __('system.parent_login_credential_report') }}</h3>
    </div>
    <div class="box-body table-responsive">
        <table class="table table-striped table-bordered table-hover parent-list">
            <thead>
                <tr>
                    <th>{{ __('system.admission_no') }}</th>
                    <th>{{ __('system.student_name') }}</th>
                    <th>{{ __('system.parent_username') }}</th>
                    <th>{{ __('system.parent_password') }}</th>
                </tr>
            </thead>
            <tbody>
                @if($searched)
