...
<div class="box box-primary">
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-md-6">
                <form method="post" action="{{ route('finance.income.search') }}">
                    @csrf
                    <input type="hidden" name="button_type" value="search_filter">
                    <div class="form-group">
                        <label>{{ __('system.search_type') }} <small class="req">*</small></label>
                        <select class="form-control" name="search_type" id="income_search_type" required>
                ...
                <form method="post" action="{{ route('finance.income.search') }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="search_text" class="form-control" required
                               ...
                               placeholder="{{ __('system.search_by_income') }}">
                    </div>
...
@if($searched)
    <div class="box box-info">
        <div class="box-header with-border">
            <h3 class="box-title"><i class="fa fa-money"></i> {{ __('system.income_list') }}</h3>
            @if(($result['mode'] ?? '') === 'filter' && ! empty($result['date_from']))
                <p class="help-block" style="margin:8px 0 0;">
                    Income Result From {{ $result['date_from'] }} To {{ $result['date_to'] }}
                </p>
            @endif
        </div>
...
<script>
    document.getElementById('income_search_type')?.addEventListener('change', function () {
