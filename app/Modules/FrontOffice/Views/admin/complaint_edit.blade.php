@php
    ...
    $val = function (string $key, $fallback = '') use ($old, $complaint_data) {
        if (array_key_exists($key, $old)) {
            return $old[$key];
        }
        if ($key === 'complaint') {
            return $complaint_data['complaint_type'] ?? $fallback;
        }

        return $complaint_data[$key] ?? $fallback;
    };
@endphp
<div class="row">
    <div class="col-md-4">
        <div class="box box-primary">
            ...
            <form action="{{ url('admin/complaint/edit/'.$complaint_data['id']) }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="box-body">
                    @if(session('success'))
                    ...
                    <div class="form-group">
                        <label>Date</label>
                        <input type="text" class="form-control" name="date" value="{{ $old['date'] ?? $complaints->formatDate($complaint_data['date'] ?? null) }}">
                    </div>
                    <div class="form-group">
