@php
    ...
    $categoryContent = $result['category_content'] ?? ['standard'];
    $selectedCategory = $old['content_category'] ?? (in_array('events', $categoryContent, true) ? 'events' : (in_array('gallery', $categoryContent, true) ? 'gallery' : (in_array('notice', $categoryContent, true) ? 'notice' : 'standard')));
    $sidebarOn = array_key_exists('sidebar', $old) ? ! empty($old['sidebar']) : ((int) ($result['sidebar'] ?? 0) === 1);
@endphp
<form action="{{ url('admin/front/page/edit/'.$result['slug']) }}" method="post">
    @csrf
