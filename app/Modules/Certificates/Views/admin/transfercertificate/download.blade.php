...
<div class="box box-primary">
    ...
    <div class="box-body">
        <form method="post" action="{{ route('certificates.tc_download.search') }}" class="row">
            @csrf
            <div class="col-sm-6">
            ...
            <table class="table table-striped table-bordered">
                ...
                @forelse($students as $student)
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="checkbox" id="is_regenerte_{{ $student->student_session_id }}" value="1">
                        </td>
                        <td>
                            <form method="post" action="{{ route('certificates.tc_download.print') }}" target="_blank" class="tc-print-form" style="display:inline;">
                                @csrf
                                <input type="hidden" name="student_id" value="{{ $student->id }}">
                                <input type="hidden" name="student_session_id" value="{{ $student->student_session_id }}">
                                <input type="hidden" name="is_regenerte" value="0" class="is-regenerte-input">
                                <button type="submit" class="btn btn-primary btn-xs" title="Download PDF">
                                    <i class="fa fa-download"></i> PDF
                                </button>
                            </form>
                            <form method="post" action="{{ route('certificates.tc_download.print_html') }}" target="_blank" class="tc-print-form" style="display:inline;margin-left:4px;">
                                @csrf
                                <input type="hidden" name="student_id" value="{{ $student->id }}">
                                <input type="hidden" name="student_session_id" value="{{ $student->student_session_id }}">
                                <input type="hidden" name="is_regenerte" value="0" class="is-regenerte-input">
                                <button type="submit" class="btn btn-default btn-xs" title="HTML print">
...
(function () {
    ...
    $(document).on('submit', '.tc-print-form', function () {
        var sessionId = $(this).find('input[name="student_session_id"]').val();
        var checked = $('#is_regenerte_' + sessionId).is(':checked');
        $(this).find('.is-regenerte-input').val(checked ? '1' : '0');
    });
})();
</script>
@endpush
