    ...
    <div class="box-header with-border">
        ...
        <div class="box-tools pull-right">
            @can('privilege', ['student_certificate', 'can_view'])
                <a href="{{ route('certificates.templates.index') }}" class="btn btn-default btn-sm">Certificate Templates</a>
            @endcan
        </div>
    </div>
    <div class="box-body">
        <form method="post" action="{{ route('certificates.generate.search') }}" class="row">
            @csrf
            <div class="col-sm-4">
                ...
                <div class="form-group">
                    ...
                    <select name="certificate_id" class="form-control" required>
                        ...
                        @foreach($certificates as $cert)
                            <option value="{{ $cert->id }}" @selected((string) ($filters['certificate_id'] ?? '') === (string) $cert->id)>
        ...
        <div class="box-header with-border">
            ...
            @if($selectedCertificate)
                <span class="text-muted" style="margin-left:8px;">
                    Template: {{ $selectedCertificate->certificate_name }}
                </span>
            @endif
        </div>
        <div class="box-body">
            <form method="post" action="{{ route('certificates.generate.print') }}" target="_blank" id="generate-print-form">
                @csrf
                <input type="hidden" name="certificate_id" value="{{ $filters['certificate_id'] }}">
                ...
                @if($students->isNotEmpty())
                    <button type="submit" class="btn btn-info btn-sm pull-right" id="generate-btn">
                        Generate
                    </button>
                @endif
            </form>
...
(function () {
    ...
    $('#generate-print-form').on('submit', function (e) {
