...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; }
        ...
        .studenttop { background: {{ $idcard->header_color ?: '#9b1818' }}; padding: 2px; color: #fff; overflow: hidden; position: relative; z-index: 1; }
...
<div class="toolbar">
    ...
    <a href="{{ route('certificates.staffidcard_templates.index') }}">Back</a>
</div>

<div class="wrap">
@if((int) $idcard->enable_vertical_card === 1)
    <table cellpadding="0" cellspacing="0" width="100%" style="background: {{ $idcard->header_color ?: '#9b1818' }}; position: relative;">
        @if($backgroundUrl)
                    ...
                    <tr>
                        <td valign="top" style="text-align:center;">
                            <div class="stimg" style="margin:0 auto;">
                                <div class="placeholder-photo" style="margin:0 auto; border:3px solid {{ $idcard->header_color ?: '#9b1818' }};">Photo</div>
                            </div>
                            <h4 style="margin:10px 0 0; text-transform:uppercase; font-weight:bold;">Staff Name</h4>
                        </td>
                    </tr>
                </table>
            </td>
            ...
            <td valign="top">
                <table cellpadding="0" cellspacing="0" width="90%" align="center" style="background:#fff; padding:20px; display:block; width:90%; margin:0 auto;">
                    <tr>
                        <td valign="top">
                            <ul class="vertlist">
                                @if((int) $idcard->enable_staff_id === 1)<li>Staff ID<span>9000</span></li>@endif
                                @if((int) $idcard->enable_designation === 1)<li>Designation<span>Teacher</span></li>@endif
                                @if((int) $idcard->enable_staff_department === 1)<li>Department<span>Admin</span></li>@endif
                                ...
                                @if((int) $idcard->enable_date_of_joining === 1)<li>Date Of Joining<span>25.06.2006</span></li>@endif
                                @if((int) $idcard->enable_permanent_address === 1)<li>Address<span>D.No.1 Street Name</span></li>@endif
                                @if((int) $idcard->enable_staff_phone === 1)<li>Phone<span>1234567890</span></li>@endif
                                @if((int) $idcard->enable_staff_dob === 1)<li>D.O.B.<span>25.06.2006</span></li>@endif
                            </ul>
                            @if($signUrl)
...
@else
    <table cellpadding="0" cellspacing="0" width="100%" style="background:#efefef; position:relative;">
        ...
        <tr>
            <td valign="top" style="color:#fff; font-size:16px; padding:4px 0; position:relative; z-index:1; background:{{ $idcard->header_color ?: '#9b1818' }}; text-transform:uppercase;">
                {{ $idcard->title }}
            </td>
        </tr>
        <tr>
            <td valign="top">
                <div class="staround">
                    <div class="cardleft">
                        ...
                        @if((int) $idcard->enable_staff_barcode === 1)
                            <div style="width:90%; margin:6px auto 0; color:#666; font-size:10px; text-align:center;">
                    ...
                    <div class="cardright">
                        <ul class="stlist">
                            @if((int) $idcard->enable_name === 1)<li>Staff Name<span>S.Tudent Name</span></li>@endif
                            @if((int) $idcard->enable_staff_id === 1)<li>Staff ID<span>9000</span></li>@endif
                            @if((int) $idcard->enable_designation === 1)<li>Designation<span>Teacher</span></li>@endif
                            @if((int) $idcard->enable_staff_department === 1)<li>Department<span>Admin</span></li>@endif
                            @if((int) $idcard->enable_fathers_name === 1)<li>Father Name<span>S.Tudent Name</span></li>@endif
