{{-- CI print/thermalPrintFeesByGroup (+ transport via FeeReceiptService line payload) --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    ...
    <div style="margin: 0 auto; padding: 0;">
        ...
        <table style="border-top: 2px #000 dashed; line-height: 11px; padding-top: 2px;">
            ...
            <tr>
                ...
                <td style="text-align:right;">{{ $formatAmount($line['balance']) }}</td>
            </tr>
        </table>

        <table style="line-height: 11px;">
            @if(empty($line['payments']))
                <tr>
                    <td colspan="5" align="left">{{ __('system.no_transaction_found') }}</td>
                </tr>
            @else
                <tr><td class="title-around-span" colspan="5"><span>{{ __('system.partial_payment') }}</span></td></tr>
                <tr>
                    <th>{{ __('system.date') }}</th>
                    <th style="text-align:right">{{ __('system.pay_id') }}</th>
                    <th style="text-align:right">{{ __('system.discount') }}({{ $currencySymbol }})</th>
                    <th style="text-align:right">{{ __('system.fine') }}({{ $currencySymbol }})</th>
                    <th style="text-align:right">{{ __('system.paid') }}({{ $currencySymbol }})</th>
                </tr>
                @foreach($line['payments'] as $payment)
                    <tr>
                        <td>{{ $payment['date'] }}</td>
                        <td style="text-align:right">{{ $payment['payment_id'] }}</td>
                        <td style="text-align:right">{{ $formatAmount($payment['amount_discount']) }}</td>
                        <td style="text-align:right">{{ $formatAmount($payment['amount_fine']) }}</td>
